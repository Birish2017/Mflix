...
namespace Migrator
{
    ...
    class Program
    {
        ...
        static string mongoConnectionString = "mongodb+srv://m001-student:m001student@berhanu-m001-ba9av.mongodb.net/<dbname>?retryWrites=true&w=majority";
        ...
        static async Task Main(string[] args)
        {
            ...
            if (datePipelineResults.Count > 0)
            {
                ...
                //
                var bulkDateWrites = new List<WriteModel<Movie>>();
                foreach(var result in datePipelineResults)
                {
                    var filter = Builders<Movie>.Filter.Eq(x => x.Id, result.Id);
                    bulkDateWrites.Add(new ReplaceOneModel<Movie>(filter,result));
                }
                bulkWriteDatesResult = await _moviesCollection.BulkWriteAsync(bulkDateWrites);
            ...
            if (ratingPipelineResults.Count > 0)
            {
                ...
                // (https://api.mongodb.com/csharp/current/html/T_MongoDB_Driver_ReplaceOneModel_1.htm).
                var ratingWriteResults = new List<WriteModel<Movie>>();
                foreach(var result in ratingPipelineResults)
                {
                    var filter=Builders<Movie>.Filter.Eq(x=>x.Id,result.Id);
                    ratingWriteResults.Add(new ReplaceOneModel<Movie>(filter, result));
                }

                bulkWriteRatingsResult = await _moviesCollection.BulkWriteAsync(ratingWriteResults);
