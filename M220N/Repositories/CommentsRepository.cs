...
namespace M220N.Repositories
{
    public class CommentsRepository
    {
        ...
        public CommentsRepository(IMongoClient mongoClient)
        {
            ...
            _usersRepository = new UsersRepository(mongoClient);
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            Builders<Comment>.Filter.Where(x => x.Id == commentId && x.Email == user.Email),
            Builders<Comment>.Update.Set(x => x.Text, comment).Set(x => x.Date, DateTime.Now),
            ...
            */
            try
            {
                ...
                result = await _commentsCollection
                  ...
                  .Aggregate()
                  .Group(label => label.Email, group => new
                  {
                      Email = group.Key,
                      Count = group.Count()
                  })
                  .Sort(new BsonDocument("Count", -1)).Limit(20)
                  .Project(x => new ReportProjection
                  {
                      Count = x.Count
                  }).ToListAsync();
