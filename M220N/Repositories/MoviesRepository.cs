...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        public async Task<Movie> GetMovieAsync(string movieId, CancellationToken cancellationToken = default)
        {
            ...
            if(string.IsNullOrWhiteSpace(movieId)|| movieId.Length!=24 || !rx.IsMatch(movieId)){
                return null;
            }
            try
            {
                return await _moviesCollection.Aggregate()
                    ...
                    m => m.Id,
                    c => c.MovieId,
                    (Movie m) => m.Comments).FirstOrDefaultAsync(cancellationToken);
                // Ticket: Get Comments
            ...
            )
        {
            ...
            //
            var result = _moviesCollection.Find(Builders<Movie>.Filter.Empty).SortByDescending(m => m.Runtime).Limit(10).ToList();
            var builder = Builders<Movie>.Filter;

            var filter = builder.AnyIn(x => x.Countries, countries);
            return await _moviesCollection
               .Find(filter)
               .SortByDescending(x => x.Title)
               .Project<MovieByCountryProjection>(Builders<Movie>.Projection.Include(x => x.Title))
               .ToListAsync(cancellationToken);
            ...
            int page = 0, params string[] genres)
        {
            ...
            return await _moviesCollection.Find(builder.AnyIn(x => x.Genres, genres))
                                          .Limit(limit)
                                          .Skip(page * limit)
                                          .Sort(sort)
                                          .ToListAsync();
